...
2. 랜덤한 세자리 숫자를 생성한다. (여기서부터 while문이 있어야한다.)
3. "숫자를 입력해주세요 : "를 출력한다. 
4. 숫자 입력을 받는다. 
5. 해당 숫자가 정답이 맞는지 확인한다. 
   1. 스트라이크 여부 확인 
   2. 볼 여부 확인
   3. 아무것도 해당되지 않았을때
    - [ ] 해당 정보를 저장하고 세개의 숫자 모두 확인한다. 
    - 정답 숫자를 기준으로 해당 숫자가 존재하는지를 확인한다.
6. 스트라이크 볼을 출력한다. 
    1. 볼이 0일 때는 출력하지 않는다. 
   2. 스트라이크가 0일때는 출력하지 않는다.
   3. 볼과 스트라이크가 0일 때는 "낫싱"을 출력한다. 
7. 스트라이크가 3일 경우에 "3개의 숫자를 모두 맞히셨습니다! 게임 종료"를 출력한다. 
8. "게임을 새로 시작하려면 1, 종료하려면 2를 입력하세요."를 출력한다. 
9. 1일 경우에 난수를 생성하고 "숫자를 입력해주세요 : "를 출력한다. 
10. 2일 경우에 그래도 종료한다. 
